<x-klarifikasi-layout>
        ...
        <div
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    Kelola akun dan penugasan role Penelaah.
                </p>
            </div>

            {{-- TOMBOL TAMBAH BARU --}}
            <div>
                <a href="{{ route('adminklarifikasi.penelaah.create') }}"
                    class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-theme-xs hover:bg-blue-700 transition">
    ...
    <div class="px-2 mb-4">
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            @if (session('success'))
                <div class="m-4 bg-green-100 border-l-4 border-green-500 text-green-700 p-4 rounded-lg" role="alert">
                    <p>{{ session('success') }}</p>
                </div>
            @endif
            @if (session('error'))
                <div class="m-4 bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded-lg" role="alert">
                    <p>{{ session('error') }}</p>
                </div>
            @endif

            <div class="p-4 sm:p-6">
                <table id="penelaahTable" class="min-w-full hover ">
                    <thead class="border-y border-gray-100 dark:border-gray-800">
                        <tr>
                            <th class="px-5 py-3 text-left font-bold whitespace-nowrap sm:px-6">
                                <p class="text-theme-lg text-gray-500 dark:text-gray-400">Nama Pengguna</p>
                            </th>
                            <th class="px-5 py-3 text-left font-bold whitespace-nowrap sm:px-6">
                                <p class="text-theme-lg text-gray-500 dark:text-gray-400">Email</p>
                            </th>
                            <th class="px-5 py-3 text-left font-bold whitespace-nowrap sm:px-6">
                                <p class="text-theme-lg text-gray-500 dark:text-gray-400">Status Penelaah</p>
                            </th>
                            <th class="px-5 py-3 text-left font-bold whitespace-nowrap sm:px-6">
    ...
    @push('scripts')
        ...
        <script>
            $(document).ready(function() {
                new DataTable('#penelaahTable', {
                    "processing": true,
                    "serverSide": true,
                    "pagingType": "simple_numbers",
                    "ajax": "{{ route('adminklarifikasi.penelaah.data') }}",
                    "columns": [{
                            "data": "name",
                            "name": "name"
                        },
                        ...
                        {
                            "data": "status_penelaah",
                            "name": "status_penelaah",
                            "orderable": false,
                            "searchable": false
                        },
                        {
