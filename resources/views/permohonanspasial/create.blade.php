<x-jig-layout>
    <div class="px-2 mb-4">
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] p-6"
            ...
            <form action="{{ route('permohonanspasial.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div x-show="userType" x-transition class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                        ...
                        <div>
                            ...
                            <x-text-input id="nama_pemohon" type="text" name="nama_pemohon" :value="old('nama_pemohon')"
                                required />
                        ...
                        <div x-show="userType !== 'masyarakat'">
                            ...
                            <x-text-input id="instansi" type="text" name="instansi" :value="old('instansi')"
                                x-bind:required="userType !== 'masyarakat'" />
                            <x-input-error :messages="$errors->get('instansi')" class="mt-2" />
                        </div>
                        {{-- Field Instansi 'dummy' untuk Masyarakat agar validasi lolos --}}
                        <input type="hidden" name="instansi" value="Masyarakat" x-show="userType === 'masyarakat'"
                            x-bind:disabled="userType !== 'masyarakat'">
                        ...
                        <div x-show="userType !== 'masyarakat'">
                            ...
                            <x-text-input id="nip" type="text" name="nip" :value="old('nip')"
                                {{-- Atribut required dinamis --}} x-bind:required="userType === 'pemerintah'" />
                        ...
                        <div x-show="userType !== 'masyarakat'">
                            ...
                            <x-text-input id="jabatan" type="text" name="jabatan" :value="old('jabatan')"
                                {{-- Atribut required dinamis --}} x-bind:required="userType === 'pemerintah'" />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                        ...
                        <div>
                            ...
                            <x-text-input id="nomor_surat" type="text" name="nomor_surat" :value="old('nomor_surat')"
                                required />
                        </div>
                        <div>
                            <label for="tanggal_surat"
                                ...
                                <span class="text-red-500">*</span></label>
                            <x-datepicker-input id="tanggal_surat" name="tanggal_surat" :value="old('tanggal_surat')" required />
                        </div>
                        <div class="col-span-1 md:col-span-2">
                            <label for="perihal"
                                class="block text-sm font-medium text-gray-700 mb-1 dark:text-gray-400">Perihal</label>
                            <textarea id="perihal" name="perihal" rows="3" class="block mt-1 w-full rounded-lg border ...">{{ old('perihal') }}</textarea>
                        </div>
                        <div class="col-span-1 md:col-span-2">
                    ...
                    <div class="mt-4 pt-4 border-t dark:border-gray-700">
                        ...
                        @if (isset($selectedIgt) && $selectedIgt->isNotEmpty())
                            <div class="space-y-4">
                                @foreach ($selectedIgt as $index => $igt)
                                    <div
                                        ...
                                        <div class="lg:col-span-1">
                                            ...
                                            <select id="cakupan_{{ $index }}"
                                                ...
                                                @foreach ($cakupanOptions as $cakupan)
                                                    <option value="{{ $cakupan }}">{{ $cakupan }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                        ...
                        @else
                            {{-- Tampilkan error jika tidak ada data IGT terpilih --}}
                            <div class="p-4 bg-red-100 border border-red-400 text-red-700 rounded-lg mt-1">
                                <p class="font-bold">Tidak ada data IGT yang dipilih.</p>
                                <p class="text-sm">Silakan kembali ke halaman <a
                                        href="{{ route('daftarigt.index') }}" class="font-medium underline">Daftar
                                        IGT</a>.</p>
                            </div>
                        @endif
                    </div>
